...
function matchingDepartures(
  ...
  return (departures.get(segment.fromStop.siteId) ?? [])
    .filter(
      (d) =>
        d.line === segment.line && d.direction_code === segment.direction.code,
    )
...
export function computeArrivalSummary(
  ...
  for (let i = 0; i < route.segments.length; i++) {
    ...
    const nextDeparture = matchingDepartures(segment, departures, now).find(
      (dep) => departureTimestamp(dep, now) >= readyAt,
    );
    ...
    if (i > 0) {
      const slackMinutes = Math.max(
        0,
        Math.floor((boardedAt - readyAt) / 60_000),
      );
      minTransferSlackMinutes =
        minTransferSlackMinutes === null
          ? slackMinutes
