...
import { getLocale, getT, resolveLocale, setLocale } from './localeStore.svelte';
...
describe('getLocale() store state transitions', () => {
  ...
  it('t stays in sync with getLocale() after multiple transitions', () => {
    setLocale('en');
    expect(getT().addSegment).toBe('+ Add segment');
    setLocale('sv');
    expect(getT().addSegment).toBe('+ Lägg till segment');
    setLocale('en');
    expect(getT().addSegment).toBe('+ Add segment');
    setLocale('sv');
    expect(getT().addSegment).toBe('+ Lägg till segment');
...
describe('getLocale() store rapid interactions', () => {
  it('handles rapid getLocale() toggles without stale intermediate state', () => {
    const sequence: ('sv' | 'en')[] = ['en', 'sv', 'en', 'sv', 'en', 'sv', 'en'];
