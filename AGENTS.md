...
```sh
pnpm install              # must use pnpm (packageManager: "pnpm@12.3.4")
...
- **pnpm only.** `packageManager: "pnpm@12.3.4"`, `node-linker=hoisted` (`.npmrc`). Stale `package-lock.json` is gitignored — delete if found.
