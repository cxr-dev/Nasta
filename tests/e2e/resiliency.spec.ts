...
test.describe("SL API Resiliency", () => {
  ...
  test("should show 'Planned' label when real-time API is empty and fallback is triggered", async ({ page }) => {
    // 1. Mock empty real-time departures
    await page.route("https://transport.integration.sl.se/v1/sites/9001/departures*", async (route) => {
      await route.fulfill({
        status: 200,
    ...
    await page.route("https://journeyplanner.integration.sl.se/v2/trip*", async (route) => {
      const futureDate = new Date(Date.now() + 10 * 60000);
      const dateStr = futureDate.toISOString().split('T')[0];
      const timeStr = futureDate.toLocaleTimeString("sv-SE", { hour: "2-digit", minute: "2-digit" });
